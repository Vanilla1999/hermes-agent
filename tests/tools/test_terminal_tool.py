...
def test_bounded_docker_availability_retries_transient_permission_denied(monkeypatch):
    results = iter([
        type("Result", (), {"returncode": 1, "stderr": "permission denied"})(),
        type("Result", (), {"returncode": 1, "stderr": "permission denied"})(),
    ...
    monkeypatch.setattr(docker_environment, "find_docker", lambda: "/usr/bin/docker")
    monkeypatch.setattr(terminal_tool.subprocess, "run", lambda *args, **kwargs: next(results))
    monkeypatch.setattr(terminal_tool.time, "sleep", calls.append)
...
def test_get_env_config_parses_strict_docker_network(monkeypatch):
    monkeypatch.setenv("TERMINAL_ENV", "docker")
    monkeypatch.setenv("TERMINAL_DOCKER_NETWORK", "false")
    assert terminal_tool._get_env_config()["docker_network"] is False

    monkeypatch.setenv("TERMINAL_DOCKER_NETWORK", "maybe")
    try:
        terminal_tool._get_env_config()
    except ValueError as exc:
        assert "TERMINAL_DOCKER_NETWORK" in str(exc)
    else:
        raise AssertionError("Docker backend must validate TERMINAL_DOCKER_NETWORK")


def test_get_env_config_ignores_invalid_docker_network_for_local_backend(monkeypatch):
    monkeypatch.setenv("TERMINAL_ENV", "local")
    monkeypatch.setenv("TERMINAL_DOCKER_NETWORK", "maybe")
    assert terminal_tool._get_env_config()["docker_network"] is True
